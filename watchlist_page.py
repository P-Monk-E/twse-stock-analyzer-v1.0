...
def add_to_watchlist(kind_or_symbol: str, symbol_or_name: Optional[str] = None,
    ...
    if any(sym == r.get("symbol", "").upper() for r in wl[key]):
        st.info("已在觀察名單中。"); return
    wl[key].append({"symbol": sym, "name": nm or sym, "added_at": datetime.now().isoformat(timespec="seconds"),
                    "pinned": False, "group": ""})
    save_watchlist(); st.success(f"已加入觀察名單：{sym}")
...
def compute_group_allocation() -> Dict[str, float]:
    pos = _load_portfolio()
    if not pos: return {"防守型":0.0,"主力":0.0,"進攻型":0.0}
    rows=[]
    for p in pos:
        grp=str(p.get("group","")).strip()
        if grp not in RECO: continue
        qty=float(p.get("qty",0))
        price=_last_price(str(p.get("symbol","")) or "")
        mv=(price or 0.0)*qty
        if mv>0: rows.append({"group":grp,"mv":mv})
    if not rows: return {"防守型":0.0,"主力":0.0,"進攻型":0.0}
    df=pd.DataFrame(rows).groupby("group",as_index=False)["mv"].sum()
    total=float(df["mv"].sum()); res={"防守型":0.0,"主力":0.0,"進攻型":0.0}
...
def _fetch_metrics(symbol: str, is_etf: bool) -> Dict[str, Any]:
    try:
        end = datetime.today(); start = end - timedelta(days=365*3); rf = 0.01
...
def _fails_stock(alpha, sharpe, de, cr, roe) -> Dict[str, bool]:
    # ✅ 門檻：Alpha>=0、Sharpe>=1、D/E<=1、CR∈[1.25,2.75]、ROE>=8%
    return {
        "Alpha": (pd.notna(alpha) and float(alpha) < ALPHA_MIN),
        "Sharpe": (pd.notna(sharpe) and float(sharpe) < SHARPE_MIN),
        "負債權益比": (pd.notna(de) and float(de) > DE_MAX),
        "流動比率": (pd.notna(cr) and (float(cr) < CR_MIN or float(cr) > CR_MAX)),
        "ROE": (pd.notna(roe) and float(roe) < ROE_MIN),
    }
        ...
        if st.button("✅ 確認執行", key="wl_ok"):
            try:
                if t=="delete":
                    ...
                    data.append({"symbol":info["symbol"].strip().upper(),"qty":int(info["qty"]),
                                 "cost":float(info["cost"]),"buy_date":info["date"],"group":info.get("group","")})
                    _save_portfolio(); st.success("已加入庫存。")
            finally:
...
def _render_watch_table(kind_key: str, is_etf_list: bool) -> None:
    wl = load_watchlist(); rows = wl[kind_key]
    if not rows: st.info("目前沒有項目。"); return
    ...
    for r in rows:
        ...
        if is_etf_list:
            ...
            fails = _fails_etf(alpha, sharpe, trey)
            lamp = "🟢" if not any(fails.values()) else "🟡"     # ETF：通過→綠燈
            out.append({
                "釘選": bool(r.get("pinned", False)), "狀態": lamp,
                "代碼": sym, "名稱": r.get("name", sym),
                "Alpha": _fmt4(alpha) + (" ❌" if fails["Alpha"] else ""),
                "Sharpe": _fmt4(sharpe) + (" ❌" if fails["Sharpe"] else ""),
                "Treynor": _fmt4(trey) + (" ❌" if fails["Treynor"] else ""),
                "Beta": _fmt4(beta), "EPS(TTM)": _fmt4(eps),
                "Score": _fmt4(score_val), "前往": f"./?nav=ETF&symbol={sym}", "🗑 刪除": False,
            })
        else:
            de = stats.get("負債權益比"); cr = stats.get("流動比率"); roe = stats.get("ROE")
            fails = _fails_stock(alpha, sharpe, de, cr, roe)
            lamp = "🟢" if not any(fails.values()) else "🟡"     # ✅ 股票：通過→綠燈
            out.append({
                "釘選": bool(r.get("pinned", False)), "狀態": lamp,
                "代碼": sym, "名稱": r.get("name", sym),
                "Alpha": _fmt4(alpha) + (" ❌" if fails["Alpha"] else ""),
                ...
                "Score": _fmt4(score_val), "前往": f"./?nav=股票&symbol={sym}", "🗑 刪除": False,
            })
    ...
    if changed: save_watchlist(); st.caption("✅ 釘選變更已自動存檔。")
    ...
    with c1:
        if st.button("刪除選取", key=f"btn_del_{kind_key}", type="secondary", disabled=(len(to_delete)==0)):
            st.session_state["wl_confirm"]={"type":"delete","kind":("etf" if kind_key=="etfs" else "stock"),"symbols":to_delete}
    with c2:
...
def _render_quick_and_alloc() -> None:
    ...
    with c1: sym = st.text_input("股票/ETF 代碼", key="qa_sym", placeholder="例：2330 或 0050")
    with c2: qty = st.number_input("股數", min_value=1, value=100, step=1, key="qa_qty")
    with c3: cost = st.number_input("成本/股", min_value=0.0, value=0.0, step=0.0001, key="qa_cost")
    with c4: dt = st.date_input("日期", value=date.today(), key="qa_date")
    with c5:
        grp = st.selectbox("分組", options=["","防守型","主力","進攻型"], key="qa_group")
        if st.button("＋ 快速存取（需確認）", key="qa_btn"):
            if not sym.strip(): st.warning("請輸入代碼。")
            else:
                st.session_state["wl_confirm"]={"type":"add_portfolio","symbol":sym.strip().upper(),
                                                "qty":int(qty),"cost":float(cost),
                                                "date":dt.isoformat(),"group":grp}
    st.markdown("---")

    alloc = compute_group_allocation()
    idx=["防守型","主力","進攻型"]
    df_chart = pd.DataFrame({"建議":[RECO[i]*100 for i in idx],
                             "目前":[alloc[i]*100 for i in idx]}, index=idx)

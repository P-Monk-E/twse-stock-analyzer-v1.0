...
from risk_grading import (
    grade_alpha, grade_sharpe, grade_debt_equity, grade_current_ratio, grade_roe, summarize,
)
...
def show(prefill_symbol: Optional[str] = None) -> None:
    ...
    st.text_input("輸入股票代碼或名稱", value=default_symbol, key="stock_symbol", on_change=_sync_symbol_from_input)
    ...
    try:
        ...
        with k3: st.metric("Beta", _fmt2(stats.get("Beta"))); st.caption("相對市場波動")
        ...
        with c1: st.metric("負債權益比", _fmt2(stats.get("負債權益比")))
        with c2: st.metric("流動比率",   _fmt2(stats.get("流動比率")))
        with c3: st.metric("ROE",       _fmt2pct(stats.get("ROE")))
        with c4: st.metric("股東權益",  _fmt0(stats.get("Equity")))
        with c5: st.metric("EPS (TTM)", _fmt2(stats.get("EPS_TTM")))

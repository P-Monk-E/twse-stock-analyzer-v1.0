...
# -------- intraday downloader (hardened) --------
def _to_tpe_naive(idx: pd.DatetimeIndex) -> pd.DatetimeIndex:
    # why: yfinance intraday 多為 UTC；先確保是 aware，再轉台北，最後去 tz
    if idx.tz is None:
...
def _download_ohlc_60m(ticker: str, period: str = "60d") -> pd.DataFrame:
    """
    強化版 60m 下載：
    1) 試 60m → 空則 fallback 30m 或 15m，再重採樣成 60m。
    2) 全程把索引轉成台北時間後去 tz，避免 rangebreaks 把資料藏掉。
    """

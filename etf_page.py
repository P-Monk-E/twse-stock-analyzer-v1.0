...
def _to_tpe_naive(idx: pd.DatetimeIndex) -> pd.DatetimeIndex:
    """索引轉成台北時間再去 tz，避免 rangebreaks 判斷錯誤。"""
    if idx.tz is None:
...
def _download_ohlc_60m(ticker: str, period: str = "60d") -> pd.DataFrame:
    """強化 60m 下載：60m→30m→15m 逐級回退，最後重採樣到 60m；統一改台北時間。"""
    def _clean(df: pd.DataFrame) -> pd.DataFrame:
        if df is None or df.empty:
            return pd.DataFrame()
        if isinstance(df.index, pd.DatetimeIndex):
            df = df.copy()
            df.index = _to_tpe_naive(df.index)
        return _ensure_ohlc(df)

    try:
        yf_t = yf.Ticker(_normalize_tw_ticker(ticker))

        df = yf_t.history(period=period, interval="60m", auto_adjust=False)
        df = _clean(df)
        if not df.empty:
            return df

        df30 = yf_t.history(period=period, interval="30m", auto_adjust=False)
        df30 = _clean(df30)
        if not df30.empty:
            return _resample_to_60m(df30)

        df15 = yf_t.history(period=period, interval="15m", auto_adjust=False)
        df15 = _clean(df15)
        if not df15.empty:
            return _resample_to_60m(df15)
...
def _market_close_series(start: pd.Timestamp, end: pd.Timestamp) -> Optional[pd.Series]:
    """取市場指數作為基準（Alpha/Sharpe/Treynor 用）。"""
...
def _prepare_tf_df(ticker: str, daily_df: pd.DataFrame, tf: str) -> Tuple[pd.DataFrame, str]:
    if tf == "60m":
        return _download_ohlc_60m(ticker, "60d"), "（60 分鐘）"
...
def render(prefill_symbol: Optional[str] = None) -> None:
    ...
    try:
        ...
        with st.container(border=True):
            ...
            # ✅ 修正：diversification_warning 需傳 (sharpe, treynor)
            warn = diversification_warning(

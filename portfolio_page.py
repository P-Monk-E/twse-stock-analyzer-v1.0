...
def _pos_neg(x: float) -> str:  # 只在 DataFrame 樣式用
    try:
        if pd.isna(x): return ""
...
def _load_portfolio() -> List[Dict[str, Any]]:
    """向下相容讀檔：接受 list 與 {"positions": list}，並在後者情況自動遷移為 list。"""
...
def show(prefill_symbol: Optional[str] = None) -> None:
    ...
    styled = (
        df.style.format(subset=["股數","成本/股","現價","市值","未實現損益","回報率%"], formatter="{:,.2f}")
        .set_properties(**{"text-align":"right"}, subset=["股數","成本/股","現價","市值","未實現損益","回報率%"])
        .hide(axis="index")
        .background_gradient(cmap="Greens", subset=["回報率%"], vmin=0, vmax=max(df["回報率%"].fillna(0).max(),1))
    ...
    try:
        sharpe, treynor, diff, dbg = estimate_portfolio_risk(df.rename(columns={"代碼":"symbol","市值":"value"}))

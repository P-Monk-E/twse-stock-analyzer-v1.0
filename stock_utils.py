...
def get_metrics(code: str, market_close: pd.Series, rf: float, start, end, is_etf: bool = False):
    ...
    # ===== 達標判斷（你強調的重點）=====
    warnings = {
        "負債權益比": debt_equity <= 1.0 if pd.notna(debt_equity) else False,
        "流動比率": current_ratio >= 1.5 if pd.notna(current_ratio) else False,
        "ROE": roe >= 0.15 if pd.notna(roe) else False,
    }
